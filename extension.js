...
function activate(context) {
    if (process.platform == 'win32') {
        ...
        function setAlpha(alpha) {
            ...
            ps.addCommand(`[VibrancyIT.SetTransParency]::SetTransParency(${process.pid}, ${alpha})`);
            ps.invoke().then(res => {
                console.log(res);
    ...
    console.log('Congratulations, your extension "VibrancyIT VSC" is now active!');

    const config = () => workspace.getConfiguration('vibrancyit');


    context.subscriptions.push(commands.registerCommand('vibrancyit.increase', () => {
        const alpha = config().get('alpha') - config().get('step');
        setAlpha(alpha);
    }));

    context.subscriptions.push(commands.registerCommand('vibrancyit.decrease', () => {
        const alpha = config().get('alpha') + config().get('step');
        setAlpha(alpha);
    }));

    context.subscriptions.push(commands.registerCommand('vibrancyit.maximize', () => {
        setAlpha(1);
    }));

    context.subscriptions.push(commands.registerCommand('vibrancyit.minimize', () => {
        setAlpha(255);
    }));
