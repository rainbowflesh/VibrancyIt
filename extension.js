...
function activate(context) {
    if (process.platform == "win32") {
        const path = context.asAbsolutePath("./vibrancyit/SetTransparency.cs");
        const ps = new shell({
        ...
        function setAlpha(alpha) {
            ...
            ps.addCommand(
                `[VibrancyIT.SetTransParency]::SetTransParency(${process.pid}, ${alpha})`,
            );
            ps.invoke()
                .then((res) => {
                    console.log(res);
                    console.log(`VibrancyIT: set alpha ${alpha}`);
                    config().update("alpha", alpha, true);
                })
                .catch((err) => {
                    console.error(err);
                    window.showErrorMessage(`VibrancyIT Error: ${err}`);
                });
        }
        ...
        // } catch (error) {
        //     console.error(`VibrancyIT Error: Please install xprop package to use VibrancyIT.`);
        //     return;
        // }
        ...
        //             if (error) {
        //                 console.error(`VibrancyIT error: ${error}`);
        //                 return;
        //             }
    ...
    console.log("VibrancyIT Running");

    const config = () => workspace.getConfiguration("vibrancyit");

    context.subscriptions.push(
        commands.registerCommand("vibrancyit.increase", () => {
            const alpha = config().get("alpha") - config().get("step");
            setAlpha(alpha);
            console.log(`VibrancyIT increase alpha to: ${alpha}`);
        }),
    );

    context.subscriptions.push(
        commands.registerCommand("vibrancyit.decrease", () => {
            const alpha = config().get("alpha") + config().get("step");
            setAlpha(alpha);
            console.log(`VibrancyIT decrease alpha to: ${alpha}`);
        }),
    );

    context.subscriptions.push(
        commands.registerCommand("vibrancyit.maximize", () => {
            setAlpha(1);
            console.log(`VibrancyIT maximize alphas: ${alpha}`);
        }),
    );

    context.subscriptions.push(
        commands.registerCommand("vibrancyit.minimize", () => {
            setAlpha(255);
            console.log(`VibrancyIT minimize alphas: ${alpha}`);
        }),
    );
